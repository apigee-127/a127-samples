...
* Obtain an access token with the OAuth 2.0 resource owner password credentials (password) grant type. In the first part of the example, we'll use Apigee Edge as the "authorization server" and then we'll show you how to use Redis running locally.
...
The OAuth 2.0 password grant type requires the client app to provide the resource owner's username/password for the resource server. After validating the user credentials, the authorization server returns an access token to the client. 

>Note: This flow should only be used for highly trusted client apps. Even though the client app has direct access to the user's credentials, the credentials are only used once in a single request, and they are exchanged for an access token. 

A client app must also supply its client ID and client secret keys to an authorization server (for example, Apigee Edge). 
...
* tokenPaths -- The paths that resolve to token endpoints on the authorization server. The endpoints generate access tokens and allow you to invalidate tokens. You can also use a refresh token to obtaion a new access token.
...
If you use the password grant type, you  must include the passwordCheck element in the x-volos-resources "oauth" definition:
...
When the authorization server gets a request for an access token, and the grant type is "password", the helper function specified in the `passwordCheck `element is automatically called. If you omit the passwordCheck element, you'll get this error:

`{"error_description":"Password check function not supplied","error":"internal_error"}`

The actual function is up to you to  implement. Here's a simple example:
...
Of course, a production implementation would validate the credentials against something like an LDAP repository or other user store. 
